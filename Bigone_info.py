...
import requests
import time
page = requests.get('https://coinmarketcap.com/exchanges/bigone/')

from bs4 import BeautifulSoup
soup = BeautifulSoup(page.content, 'html.parser')
trading_dict = dict()
market_id = 6
market_logo = 'http://p1nzzscwm.bkt.clouddn.com/bigone_logo.png'
market_url = 'https://big.one/'
...
for n in td_list:
    if 'data-sort' not in str(n):
        ...
        if ticker_count ==2:
            ...
            if ticker[1] in currencies:
                ...
                try:
                    ...
                    try:
                        ...
                        print 'cant find this coin in ccc and database'
                        continue
            ...
            collection.update_one({'name':'BigONE'},{'$set':{'trading'+'.'+ticker[1]+'.'+ticker[0]:{'coin_id':coin_id,'logo':logo,'update_time':int(time.time()),'last_price':price,'percent':percent,'volume':volume}}},upsert=True)
            
            collection = db['DAS.exchange.bycoin']
            collection.update_one({'id':coin_id},{'$set':{ticker[1]+'.'+'BigONE':{'update_time':int(time.time()),'last_price':price,'percent':percent,'volume':volume,'exchange_id':market_id,'logo':market_logo,'url':market_url}}},upsert=True)
            client.close()
